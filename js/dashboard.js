...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const badgeContainer = document.getElementById("capsuleBadge"); // Badge container placeholder div
    ...
    // Show/hide capsule section
    function tipCheck() {
        if (capsuleList && capsuleList.children.length === 0) {
            capsuleDisplay.style.display = "none";
        } else {
            capsuleDisplay.style.display = "block";
        }
    }
    ...
    if (typeof capsules !== "undefined" && Array.isArray(capsules) && capsuleList) {
        const BADGES = {
            1: {
                name: "First Capsule Badge",
                src: "/static/svg/firstCapsuleBadge.png",
                desc: "Congrats on sending your first time capsule!"
            },

            5: {
                name: "Five Capsule Badge",
                src: "/static/svg/fiveCapsuleBadge.png",
                desc: "Congrats on sending five time capsules!"
            }
        }

        for (const [key, badge] of Object.entries(BADGES)) {
            if (capsulesUploaded + capsules.length >= key) {
                const badgeElement = document.createElement("div");
                badgeElement.classList.add("badge");
                badgeElement.innerHTML = `
                    <img src="${badge.src}" alt="${badge.name}">
                    <div class="badge-content">
                        <p class="badge-text">${badge.desc}</p>
                    </div>
                `;
        ...
        // if (typeof badgeEarnedFirst !== "undefined" && badgeEarnedFirst && badgeContainer) {
        ...
        //         </div>
        //     `;
        //     badgeContainer.appendChild(firstBadgeElement);
        // }
