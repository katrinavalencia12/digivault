...
def send_scheduled_email(email, file_paths, capsule_name, messages=None):
    with app.app_context():
        try:
            ...
            # Build website
            link = build_website(user.username, capsule_name, file_paths, messages)
            ...
            <body style='margin: 0; text-align: center;'>
                ...
                <p style="margin: 40px;">Your time capsule '{capsule_name}' is ready to view.</p>
                <a href="http://{ip}/{link}" style='background-color: #2b2f4d; border: 1px solid #2b2f4d; padding: 10px; color: white; text-decoration: none; text-transform: uppercase; border-radius: 25px; margin: 40px !important; transition: all 0.3s' onmouseover="this.style.backgroundColor='transparent'; this.style.color='black';" onmouseleave="this.style.backgroundColor='#2b2f4d'; this.style.color='white';">View My Capsule</a>
            </body>
            """
            # paths = []
            # for index, path in enumerate(file_paths):
            #     filename = os.path.basename(path)
            #     username_dir = os.path.basename(os.path.dirname(path))
            #     download_link = f"http://{ip}/uploads/{username_dir}/{filename}"
            #     paths.append(download_link)
            #     file_message = ""
            #     if messages and str(index) in messages:
            #         file_message = f"<br><em>Message: {messages[str(index)]}</em>"
            #     body_html += f'<li><a href="{download_link}" target="_blank">{filename}</a>{file_message}</li>'
            # body_html += "</ul>"
            ...
            # ADD THIS BLOCK to record to CapsuleHistory:
            if user:
...
def login():
    ...
    if request.method == 'POST':
        ...
        # If user exists and password matches hashed password, log them in
    ...
    try:
        ...
        for email in recipient_emails:
            scheduler.add_job(
                ...
                args=[email, file_paths, name, messages],
